...
def worst_daily_comments(request):
    ...
    try:
        ...
        raise Http404
    worst_daily_comments = get_ranked_hash(worst_daily_comments, lambda comment: comment.downvotes)
    return render_to_response('bwog/ranked.html', {
        'description': 'Worst Comments',
        'timeframe': 'Today',
        'rank_unit': 'Downvotes',
...
def correlation(request):
    ...
    #    term1_worst_body = cache_count_select(connection,["SELECT id FROM main_bwogcomment WHERE downvotes = (SELECT max(downvotes) from main_bwogcomment where body ILIKE %s)", ['%' + term1 + '%']])
        ...
        if term1_count > 0:
            ...
            term1_average_upvotes = 0
            term1_average_downvotes = 0
        ...
        if term2_count > 0:
            ...
            term2_average_upvotes = 0
            term2_average_downvotes = 0

        if term2_average_upvotes > 0:
            upvote_ratio = round((float(term1_average_upvotes) / float(term2_average_upvotes)), 2)
            downvote_ratio = round((float(term1_average_downvotes) / float(term2_average_downvotes)), 2)
        else:
            upvote_ratio = 0
            downvote_ratio = 0
        ...
        if term2_prob > 0:
            ...
            term1_given_term2_prob = 0

        if term1_prob > 0:
            term2_given_term1_prob = both_prob / term1_prob
        else:
            term2_given_term1_prob = 0
        ...
        if term1_variance > 0 and term2_variance > 0:
