God.watch do |w|
  ...
  w.start = "cd #{root} && . #{root}/venv/bin/activate && /usr/local/sbin/daemonize -a -v -c #{root} -o #{root}/run/gunicorn.log -e #{root}/run/gunicorn.log -p #{pid_file} #{gunicorn} -w 5 hivemindio.wsgi:application &>> #{root}/run/gunicorn.log"
  w.dir = root
